...
local function piranha_ff(self)
	for i = 1,#water_life.shark_food,1 do
		if water_life.shark_food[i] ~= "water_life:fish" and water_life.shark_food[i] ~=
			"water_life:fish_tamed" then
...
local function piranha_brain(self)
	...
	if mobkit.timer(self,1) then 
        ...
        if target then
			...
			if dist > 2 and mobkit.is_alive(target) and mobkit.is_in_deep(target)
				and target:get_attach() == nil then
					mobkit.clear_queue_high(self)
					water_life.hq_water_attack(self,target,50,5)
			elseif target and mobkit.is_alive(target) and ((not mobkit.is_in_deep(target))
				or (target:get_attach() ~= nil)) then
        ...
        if self.isinliquid and self.isinliquid == "default:water_source" then
			water_life.hq_swimto(self,20,1,{"water_life:muddy_river_water_source",
